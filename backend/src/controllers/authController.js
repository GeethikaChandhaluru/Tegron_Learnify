...
const signup = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      data: { ...safeUser(user), token: generateToken(user._id, user.role) },
    });
  } catch (error) {
...
const resetPassword = async (req, res) => {
  try {
    ...
    return res.json({ success: true, message: 'Password reset successful', token: generateToken(user._id, user.role) });
  } catch (error) {
...
// @access  Private
// Body: { username?: string, profilePic?: string (base64 data URL) }
const updateProfile = async (req, res) => {
  try {
    ...
    // ── Profile picture: stored as base64 data URL directly in MongoDB ────────
    // No filesystem required — works on Render free tier & any hosting
    if (req.body.profilePic && req.body.profilePic.startsWith('data:image/')) {
      // Optional: basic size guard (~1MB base64 ≈ 750KB actual)
      if (req.body.profilePic.length > 1_500_000) {
        return res.status(400).json({ success: false, message: 'Image too large. Please use an image under 1MB.' });
      }
